...
export const AGENDAMENTO: Fluxo = {
  id: 'agendamento',
  nome: 'Confirmação de visita',
  descricao: 'Confirma que o cliente estará no local na data marcada.',
  etapas: [
    { id: 'abertura', rotulo: 'Abertura' },
    { id: 'confirmado', rotulo: 'Confirmação' },
    { id: 'doc_enviado', rotulo: 'Lembrete' },
    { id: 'fim', rotulo: 'Encerrada' },
  ],
  campos: [
    { nome: 'os_numero', rotulo: 'Ordem de serviço', tipo: 'texto', obrigatorio: true, exemplo: '4181234567' },
    { nome: 'cliente_nome', rotulo: 'Nome do cliente', tipo: 'texto', obrigatorio: true },
    { nome: 'telefone', rotulo: 'Telefone com DDD', tipo: 'telefone', obrigatorio: true, exemplo: '79999998888' },
    { nome: 'cliente_endereco', rotulo: 'Endereço da visita', tipo: 'area', obrigatorio: true },
    { nome: 'data_visita', rotulo: 'Data da visita', tipo: 'texto', obrigatorio: true, exemplo: 'quinta 24/07', meio: true },
    { nome: 'turno', rotulo: 'Turno', tipo: 'select', obrigatorio: true, opcoes: [
      { valor: 'manha', rotulo: 'Manhã (8h-12h)' },
      { valor: 'tarde', rotulo: 'Tarde (13h-18h)' },
    ] },
...
Ordem de serviço: ${d.os_numero}
Cliente: ${d.cliente_nome}
Endereço: ${d.cliente_endereco}
Data: ${d.data_visita}
Turno: ${d.turno === 'manha' ? 'manhã, entre 8h e 12h' : 'tarde, entre 13h e 18h'}

# Roteiro

## 1. Abertura
Identifique-se, diga a empresa e o número da OS, avise que a ligação é gravada. Pergunte se pode continuar.
Se não for o titular e ele não puder atender, encerre com status nao_e_o_titular.
...
${d.produto_linha === 'RAC' ? 'Para ar-condicionado, avise que é preciso haver acesso à unidade externa.' : ''}
Pergunte se prefere receber os detalhes por WhatsApp ou SMS e chame enviar_link_documentos.

## 4. Encerramento
Resuma em uma frase e chame encerrar_triagem.

# Limites
Não prometa horário exato dentro do turno.
Não fale valores.`,
};

export const DOCUMENTACAO: Fluxo = {
  id: 'documentacao',
  nome: 'Cobrança de documentação',
  descricao: 'Cobra a nota fiscal ou a etiqueta que ainda não chegaram.',
  etapas: [
    { id: 'abertura', rotulo: 'Abertura' },
    { id: 'confirmado', rotulo: 'Motivo' },
    { id: 'doc_enviado', rotulo: 'Novo link' },
    { id: 'fim', rotulo: 'Encerrada' },
  ],
  campos: [
    { nome: 'os_numero', rotulo: 'Ordem de serviço', tipo: 'texto', obrigatorio: true },
    { nome: 'cliente_nome', rotulo: 'Nome do cliente', tipo: 'texto', obrigatorio: true },
    { nome: 'telefone', rotulo: 'Telefone com DDD', tipo: 'telefone', obrigatorio: true },
    { nome: 'pendencia', rotulo: 'O que falta', tipo: 'select', obrigatorio: true, opcoes: [
      { valor: 'nota', rotulo: 'Nota fiscal' },
      { valor: 'etiqueta', rotulo: 'Foto da etiqueta' },
      { valor: 'ambos', rotulo: 'Nota fiscal e etiqueta' },
    ] },
...
Ordem de serviço: ${d.os_numero}
Cliente: ${d.cliente_nome}
Falta: ${{ nota: 'a nota fiscal de compra', etiqueta: 'a foto da etiqueta de identificação do produto', ambos: 'a nota fiscal e a foto da etiqueta de identificação' }[d.pendencia] ?? 'documentação'}
Parada há: ${d.dias_parado} dias
...
## 2. O motivo
Diga com clareza e sem culpar o cliente que a OS está parada há ${d.dias_parado} dias esperando o documento, e que o atendimento só avança depois disso.
Pergunte se ele chegou a receber o link. Escute o motivo: não recebeu, não achou a nota, não sabe onde fica a etiqueta.
Se disser que não sabe onde fica a etiqueta, explique: é uma etiqueta prateada ou branca colada no próprio aparelho, geralmente atrás ou na lateral, com o número de série.
Se disser que perdeu a nota fiscal, oriente a procurar no e-mail da compra ou no aplicativo da loja.
Chame confirmar_cadastro registrando no campo restricao_horario o motivo que ele deu.

## 3. Novo link
Pergunte por qual canal prefere e confirme o número. Chame enviar_link_documentos.

## 4. Encerramento
Diga que assim que chegar o documento a OS volta a andar. Agradeça e chame encerrar_triagem.

# Limites
Nunca cobre com tom de reclamação. O cliente pode ter motivo legítimo.
Se ele disser que desistiu, encerre com status parcial e observação "desistiu".`,
};

export const SATISFACAO: Fluxo = {
  id: 'satisfacao',
  nome: 'Pesquisa de satisfação',
  descricao: 'Ouve o cliente depois do atendimento concluído.',
  etapas: [
    { id: 'abertura', rotulo: 'Abertura' },
    { id: 'confirmado', rotulo: 'Nota' },
    { id: 'sintoma_ok', rotulo: 'Comentário' },
    { id: 'fim', rotulo: 'Encerrada' },
  ],
  campos: [
    { nome: 'os_numero', rotulo: 'Ordem de serviço', tipo: 'texto', obrigatorio: true },
...
Ordem de serviço: ${d.os_numero}
Cliente: ${d.cliente_nome}
Serviço: ${d.servico_realizado}
${d.tecnico ? `Técnico: ${d.tecnico}` : ''}

# Roteiro

## 1. Abertura
Identifique-se, diga a empresa e o número da OS, avise que a ligação é gravada. Diga que a conversa leva menos de dois minutos e pergunte se pode continuar.
Se disser que não pode agora, encerre com status parcial e observação "pediu para ligar depois". Não insista.

## 2. A nota
Pergunte, de zero a dez, o quanto ele recomendaria a Smart Center para um amigo.
Espere a nota. Se der um intervalo ou hesitar, peça um número.
Chame confirmar_cadastro registrando a nota no campo restricao_horario, no formato "NPS: 8".

## 3. O comentário
Se a nota for 9 ou 10: pergunte o que mais funcionou bem.
Se for 7 ou 8: pergunte o que faltou para ser dez.
Se for 6 ou menos: peça desculpas com sinceridade, pergunte o que deu errado, e escute sem interromper nem justificar. Ao final, chame transferir_humano com motivo insatisfacao.
Chame registrar_sintoma com o comentário no campo sintoma_confirmado e frequencia "nao_informado".

## 4. Encerramento
Agradeça pelo tempo e chame encerrar_triagem.

# Limites
Nunca discuta a nota nem tente convencer o cliente a mudá-la.
Não prometa retorno, desconto ou compensação.
